...
public class Game
{
	...
	private void generateApple()
	{
		...
		while ((collission(apple) == 1)||(appleNullZone(apple)))
		{
			random 	= new Random	();
			apple 	= new Point		(random.nextInt(19 - 0), random.nextInt(19 - 0));
		}
	...
	// Keep apple out of corners
	private boolean appleNullZone(Point apple)
	{
		// This switch checks the four corners of the border
		// (a player can turn tight enough to get these locations)
		switch (apple.x)
		{
			case 0:
				if ((apple.y == 0)
			...
			// From here it is checking the walls inner corners in map 3
			case 4:
				if ((apple.y == 5)
				  ||(apple.y == 6)
				  ||(apple.y == 13)
